...
<style>
    ...
    .order-item { border-left: 4px solid var(--bull-red); background: var(--background-color); border-radius:10px; padding:16px; margin-bottom:12px; }
    ...
    .focus-box { background: rgba(245,158,11,0.06); border-left:4px solid var(--focus-amber); border-radius:8px; padding:16px; }
...
if "决策大屏" in page:
    ...
    <div class="flex-header">
        ...
        <span class="date-badge">基于 {v3_state.get('date', today_str) if v3_state else today_str} 盘后数据</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not os.path.exists(report_file):
        st.warning("今日日报尚未生成，请等待 GitHub Actions 自动运行（交易日 8:30 后生成）。")
    else:
        ...
        if orders:
            orders_clean = re.sub(r'\n?\s*【今日ETF排行榜.*', '', orders, flags=re.DOTALL)
            orders_clean = re.sub(r'\n\s*\d+\..*S=\d+\.\d+', '', orders_clean)
            st.markdown("### 📋 今日交易指令")
            st.markdown(f'<div class="za-card">{orders_clean.strip().replace(chr(10), "<br>")}</div>', unsafe_allow_html=True)
        ...
        with open(report_file, encoding='utf-8') as f:
            for line in f:
                m = re.match(r'\s*(\d+)\.\s+(.+?)\s+\((\d+\.(?:SZ|SH))\)\s+S=([\d.]+)', line)
                if m:
...
# ----------------- 资产账本 -----------------
    ...
    <div class="za-card">
        <div>总资产净值</div>
        <div style="font-size:2.8rem; font-weight:800;">{total_assets:,.2f} CNY</div>
    ...
    if portfolio['holdings']:
        ...
        <th>标的资产</th><th>持有数量</th><th>持仓均价</th><th>最新现价</th><th>止盈</th><th>止损</th><th>浮动盈亏</th>
        </tr></thead><tbody>'''
        for c, h in portfolio['holdings'].items():
            curr_p = get_latest_price(c) or h['cost']
